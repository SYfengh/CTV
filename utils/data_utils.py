...
def main():
    ...
    parser.add_argument("--in_dir", type=str, default="dataset/retacred")
    parser.add_argument("--out_dir", type=str, default="dataset/retacred")
    ...
    for split in ["dev", "train", "test"]:
        ...
        elif args["data_name"] == "semeval" or args["data_name"] == "2010i2b2" or args["data_name"] == "wiki80":
            pass
        else:
    ...
    sample_from_data(out_dir / f"train-{initial_train_ratio}.json", initial_train_ratio, [0.03,0.05,0.1, 0.15,0.3])
    # sample_from_data(out_dir / f"raw-{initial_raw_ratio}.json", initial_train_ratio, [])


if __name__ == "__main__":
    # wiki80数据集，从训练集中再划出一部分作为验证集
    
    # data = [json.loads(line) for line in open('dataset/wiki80/train.txt', "r")]
    # print(len(data))
    # idxs = set(random.sample(range(630), round(70)))
    # train_data = []
    # dev_data = []
    # for i in range(80):
    #     offset = i * 630
    #     for idx in range(630):
    #         if idx not in idxs:
    #             train_data.append(data[idx + offset])
    #         else:
    #             dev_data.append(data[idx + offset])
    
    # print(len(train_data))
    # print(len(dev_data))
    # # 写入文件
    # with open('dataset/wiki80/train.txt', 'w') as w:
    #     for line in train_data:
    #         w.write(json.dumps(line, ensure_ascii=True))
    #         w.write('\n')
    # with open('dataset/wiki80/dev.txt', 'w') as w:
